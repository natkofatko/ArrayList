...
public class Quadratic implements Comparable<Quadratic> {
	 ...
	 * 
	 * @return
	 * Calculate the focus of the parabola based on the vertex 
	 */
	public double [] getFocus()
	{
		double x1 = -b / (2 * this.a);
		double z = this.a * Math.pow(x1, 2) + this.b * x1 + this.c;
		double d=1/(4*this.a); 
		double y1=z+d; 
		return new double [] {x1,y1};
	}
	 ...
	 * @return
	 *
	 * calculate the dirextrix of the parabola based on the length d and vertex
	 */
	public double getDirectrix(){
		double x1 = -b / (2 * this.a);
		double y1 = this.a * Math.pow(x1, 2) + (this.b * x1) + this.c;
		double d=1/(4*this.a);
		double directrix=y1-d; 
		return directrix; 
